...
import { dictionaryExport } from '../portability';
import { seedDictionaries } from '../../db/defaults';
afterEach(() => { vi.unstubAllGlobals(); vi.useRealTimers(); });
it('awaitを挟まずユーザー操作内で共有を呼ぶ', async () => {
  const share = vi.fn().mockResolvedValue(undefined);
  vi.stubGlobal('navigator', { canShare: () => true, share });
  const result = shareExport(dictionaryExport(seedDictionaries()), 'test.json');
...
it.each(['missing', 'false', 'throws'])('共有判定%sならダウンロードへフォールバックする', async (mode) => {
  ...
  await shareExport(dictionaryExport(seedDictionaries()), 'test.json'); expect(click).toHaveBeenCalledOnce(); expect(share).not.toHaveBeenCalled();
...
it('共有中止は無通知、共有失敗は呼び出し元へ返す', async () => {
  ...
  vi.stubGlobal('navigator', { canShare: () => true, share });
  await expect(shareExport(dictionaryExport(seedDictionaries()), 'test.json')).resolves.toBeUndefined();
  await expect(shareExport(dictionaryExport(seedDictionaries()), 'test.json')).rejects.toThrow('failed');
});
