...
const BOARD_DATE = '2026-09-09';
const BOARD_LABEL = '2026年9月9日';

const chip = (id: string, rawText: string, quadrant: MemoItem['quadrant']): MemoItem =>
  ({ id, boardDate: BOARD_DATE, rawText, normText: rawText, quadrant, matchedEntry: null, autoClassified: false, createdAt: 1, updatedAt: 1 });
...
it('メモ0件でも4象限のラベルを空の象限として返す', () => {
  expect(boardImageLayout([], BOARD_DATE)).toEqual([
...
it('辞書を必要とせず空の象限も固定ラベルで描画対象に残す', () => {
  const layout = boardImageLayout([chip('1', 'メモ', 'q4')], BOARD_DATE);
...
it('PNGのFileを同期的に返し、全象限のラベルとチップ本文を描く', () => {
  ...
  const file = boardImageFile(boardImageLayout([chip('1', '会議', 'q1'), chip('2', '牛乳', 'q3')], BOARD_DATE), 'quadmemo-board-2026-09-09.png', BOARD_DATE);
...
it('長い本文は折り返して全文を描き、切り捨てない', () => {
  ...
  boardImageFile(boardImageLayout([chip('1', long, 'q1')], BOARD_DATE), 'board.png', BOARD_DATE);
...
it('全固定ラベルの描画幅に余裕があり、maxWidth の保険も維持する', () => {
  ...
  boardImageFile(boardImageLayout([], BOARD_DATE), 'board.png', BOARD_DATE);
...
it('メモ0件でもPNGを返しエラーにならない', () => {
  const { texts } = stubCanvas();
  const file = boardImageFile(boardImageLayout([], BOARD_DATE), 'board.png', BOARD_DATE);
  expect(file.type).toBe('image/png');
  expect(texts).toEqual([BOARD_LABEL, 'Q2', '野菜', 'Q1', 'それ以外', 'Q3', '肉類・乳製品', 'Q4', 'ドラッグストア']);
});
it('roundRect 非対応の環境では rect のフォールバックでチップ枠を描く', () => {
  const { texts, rectCalls } = stubCanvas(true);
  const file = boardImageFile(boardImageLayout([chip('1', '会議', 'q1')], BOARD_DATE), 'board.png', BOARD_DATE);
  expect(file.type).toBe('image/png');
...
it('2Dコンテキストを取得できない環境では例外を投げる', () => {
  ...
  expect(() => boardImageFile(boardImageLayout([], BOARD_DATE), 'board.png', BOARD_DATE)).toThrow(/この環境ではキャンバスを利用できません/);
});
it.each(['data:,', 'data:image/png;base64,'])('toDataURL が %s を返す環境では例外を投げる', (dataUrl) => {
  ...
  expect(() => boardImageFile(boardImageLayout([], BOARD_DATE), 'board.png', BOARD_DATE)).toThrow(/画像を書き出せませんでした/);
});
...
it('1文字チップ50個を各象限に置いても日付の帯を含めて面積上限に収まる', () => {
  stubCanvas();
  ...
  boardImageFile(boardImageLayout(chips, BOARD_DATE), 'board.png', BOARD_DATE);
  // 象限の最悪ケース 8266px（add-quadmemo-memo-length-limit の計算）＋ 日付の帯 96px。
  expect(size).toEqual({ width: 1080, height: 8266 + 96 });
  expect(size.width * size.height).toBeLessThan(16_777_216);
});

it('表示中のボードの日付のチップだけを返す', () => {
  const layout = boardImageLayout([
    chip('1', '当日のメモ', 'q1'), { ...chip('2', '前日のメモ', 'q1'), boardDate: '2026-09-08' },
    { ...chip('3', '別日の牛乳', 'q3'), boardDate: '2026-09-06' },
  ], BOARD_DATE);
  expect(layout.map(({ quadrant, texts }) => ({ quadrant, texts }))).toEqual([
    { quadrant: 'q2', texts: [] }, { quadrant: 'q1', texts: ['当日のメモ'] },
    { quadrant: 'q3', texts: [] }, { quadrant: 'q4', texts: [] },
  ]);
  // 過去のボードを対象にすればその日付のチップだけになる。
  const past = boardImageLayout([
    chip('1', '当日のメモ', 'q1'), { ...chip('2', '前日のメモ', 'q1'), boardDate: '2026-09-08' },
  ], '2026-09-08');
  expect(past[1].texts).toEqual(['前日のメモ']);
});
it('画像に対象ボードの年月日を描く', () => {
  const { texts, fillTextCalls } = stubCanvas();
  boardImageFile(boardImageLayout([chip('1', '会議', 'q1')], BOARD_DATE), 'board.png', BOARD_DATE);
  expect(texts[0]).toBe(BOARD_LABEL);
  // 帯の幅に収める保険。
  expect(fillTextCalls[0].maxWidth).toBe(1080 - 32 * 2);
  const past = stubCanvas();
  boardImageFile(boardImageLayout([], '2026-09-06'), 'board.png', '2026-09-06');
  expect(past.texts[0]).toBe('2026年9月6日');
});
