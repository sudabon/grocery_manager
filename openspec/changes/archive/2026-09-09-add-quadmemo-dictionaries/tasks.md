...
- [x] 4.1 iPhone 実機でエクスポートから共有シートが開き、ファイルアプリ等へ保存できることを確認
- [x] 4.2 保存したファイルを iPhone 実機でインポートし、メモ・辞書・設定が復元されることを確認（仕様書 §15 の受け入れ基準）
- [x] 4.3 設定画面のストレージ永続化結果が実機の状態を反映していることを確認
- [x] 4.4 数千語規模の単語リストを保存し、保存操作の応答が実用範囲であることを確認
