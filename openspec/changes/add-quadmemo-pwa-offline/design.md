...
`setup-quadmemo-hosting` の配信側は既に「エントリポイント（`index.html` / `sw.js` / `registerSW.js` / `manifest.webmanifest`）は `no-cache`、ハッシュ付きアセットは 1 年 immutable」で構成済みで、本 change で初めて `sw.js` 系が実体化する（同 change design - D4 の宿題を回収する）。
...
- UI（`UpdateToast`）は「通知を出す」「更新を実行する」しか知らない。これにより E2E とユニットテストから更新イベントを注入しやすくなる
- 更新実行は `updateServiceWorker(true)`（`skipWaiting` → `clients.claim` → リロード）

### D3: アイコンはコミット済み PNG + 生成元 SVG（ビルド時生成にしない）

- `public/icons/` に `icon-192.png` / `icon-512.png` / `maskable-512.png` / `apple-touch-icon-180.png` と生成元の `icon.svg` を置く
...
`add-quadmemo-dictionaries` が作ったアプリ情報欄に「Service Worker: 登録済み / 更新待機中 / 未登録」を足す。取得元は `navigator.serviceWorker.getRegistration()` と D2 の購読状態。
...
`setup-quadmemo-hosting` design - D4 の宿題。`npm run build` の実出力ファイル名（`sw.js` / `registerSW.js` / `manifest.webmanifest`）と `scripts/deploy.sh` の除外リストを突き合わせ、デプロイ後に `curl -I` で `cache-control: no-cache` を実測する。ここが崩れると以後の更新が一切届かなくなるため、受け入れタスクとして明示する。
