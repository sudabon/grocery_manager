...
export class DictionariesPage {
  ...
  get textboxes() { return this.page.getByRole('textbox'); }
  ...
  tab(q: number) { return this.page.getByRole('tab', { name: new RegExp(`^Q${q} `) }); }
  async open() { await this.page.getByRole('link', { name: '辞書編集', exact: true }).click(); await expect(this.entries).toBeVisible(); }
  ...
  async reload() { await this.page.reload(); await expect(this.entries).toBeVisible(); }
